...
int main(){
  ...
  while(match == 0 && chance_count <= MAX_CHANCES/*as long as match is false and chances are <= MAX_CHANCES*/){ // 20 Marks
    ...
    if (user_guess > computer_guess/* write the condition */){                     // 10 Marks
  ...
  //********************* EXTRA CREDIT ************************/
  if (chance_count <= 4) printf("You are Super Duper NINJA!\n");
  else if (chance_count <= 6) printf("You are Crezy NINJA!\n");
  else if (chance_count <= 8) printf("You have potential to be a NINJA!\n");
  printf("\n");
  ...
  if (chance_count > (MAX_CHANCES + 1)/* complete the condition */){                // 10 marks
    printf(" Try to be Mathemetical.\n");// print the game over message.                 // 5 marks
  }
  printf("\n");
  return 0;
}
